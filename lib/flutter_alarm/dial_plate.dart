...
class DialPlate extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double i = 0; i < _numPoint; i++) {
      ...
      //     ]
      // ));
      _timeParagraphBuilder.addText(_getTimeStr(hour, minute, second));

      Paragraph paragraph = _timeParagraphBuilder.build();
      paragraph.layout(ParagraphConstraints(width: 230));
      canvas.drawParagraph(paragraph, Offset(-115, -22.5));
      canvas.restore();
    }
  ...
  ///获取时间的字符串形式
  String _getTimeStr(int hour, int minute, int second) {
    String hourStr;
    String minuteStr;
    String secondStr;
    if (hour < 10)
